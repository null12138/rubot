...
async fn send_rich_message(
    ...
    // Sending mixed text+media in one call results in media being silently dropped.
...
/// Send a single item as its own message. This matches the official pattern where
/// each text/image/file item is sent in a separate API call with a single-item item_list.
...
/// Start the WeChat iLink channel with auto-reconnect.
pub async fn start(agent: Arc<Mutex<agent::Agent>>, cfg: crate::config::Config) -> Result<()> {
    if cfg.wechat_bot_token.is_empty() {
        tracing::info!("wechat bot: disabled (no bot_token)");
        return Ok(());
    }
    ...
    loop {
        let mut bot = WeChatBot::new(
            cfg.wechat_bot_token.clone(),
            cfg.wechat_base_url.clone(),
            agent.clone(),
        ...
        match bot.run().await {
            Ok(()) => {
                tracing::info!("wechat bot: poll loop ended cleanly, reconnecting...");
            }
            Err(e) => {
                tracing::warn!("wechat bot: session error: {:#}, reconnecting in 30s...", e);
            }
        }
