...
impl ToolCallChain {
    ...
    pub fn add_step(&mut self, tool: &str, params: serde_json::Value, desc: &str, deps: Vec<usize>) -> usize {
        ...
        self.steps.push(ChainStep {
            id, tool: tool.into(), params, desc: desc.into(),
            depends_on: deps, status: StepStatus::Pending, result: None,
        });
        id
    }
    ...
    pub fn next_ready(&self) -> Option<usize> {
        self.steps.iter().find(|s| s.status == StepStatus::Pending
            && s.depends_on.iter().all(|&id| self.steps.get(id).is_some_and(|d| d.status == StepStatus::Done)))
            .map(|s| s.id)
    }
    ...
    pub fn resolve(&self, params: &serde_json::Value) -> serde_json::Value {
        ...
        for step in &self.steps {
            if let Some(r) = &step.result {
                s = s.replace(&format!("$step_{}.result", step.id),
                    &r.replace('\\', "\\\\").replace('"', "\\\""));
            }
        }
...
impl<'a> ChainExecutor<'a> {
    pub fn new(registry: &'a ToolRegistry, retries: u32) -> Self {
        Self { registry, retries }
    }
