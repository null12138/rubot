...
use crate::tools::{
    code_exec::CodeExec, file_ops::FileOps, latex_pdf::LatexPdf,
...
impl Agent {
    pub async fn new(config: Config) -> Result<Self> {
        let llm = LlmClient::new(
            ...
            &config.model, &config.fast_model, config.max_retries,
        );
        ...
        tools.register(Box::new(CodeExec::new(config.code_exec_timeout_secs, &config.workspace_path))).await;
        tools.register(Box::new(FileOps::new(&config.workspace_path))).await;
        tools.register(Box::new(LatexPdf::new(&config.workspace_path))).await;
        tools.load_md_tools().await?;

        let memory = MemorySearch::new(&config.workspace_path);

        let memory_index = memory.get_index_text().await.unwrap_or_else(|_| "(empty)".into());
        let system_msg = Message::system(&personality::system_prompt(&memory_index));
    ...
    async fn run_loop(&mut self) -> Result<String> {
        loop {
            ...
            if self.iteration_count > self.max_iterations {
                ...
                return Ok(format!("Reached maximum iterations ({}) without converging.", self.max_iterations));
            }
            ...
            let response = if is_first_round {
                self.llm.chat(&self.messages, Some(&tool_defs), Some(temp)).await
            } else {
                self.llm.chat_fast(&self.messages, Some(&tool_defs), Some(temp)).await
            ...
            if !is_first_round && response_text.trim().len() < 200 {
                ...
                let final_text = resp.choices.into_iter().next()
    ...
    async fn execute_tools(&self, tool_calls: &[ToolCall]) -> Result<Vec<(ToolCall, ToolResult)>> {
        ...
        for tc in tool_calls {
            ...
            println!("  {}→{} {}{}{} {}{}{}", DIM, R, CYAN, tc.function.name, R, DIM, summary, R);
            let result = match self.tools.execute(&tc.function.name, params).await {
                Ok(r) => r,
                Err(e) => ToolResult::err(format!("{:#}", e)),
            };
            let (mark, color) = if result.success { ("✓", GREEN) } else { ("✗", RED) };
            let raw = if result.success { result.output.as_str() } else { result.error.as_deref().unwrap_or("") };
            let preview: String = raw.lines()
                .find(|l| l.trim().len() >= 4)
    ...
    async fn handle_plan(&mut self, mut chain: ToolCallChain) -> Result<String> {
        ...
        for (step_id, output) in &outputs {
            ...
            let suffix = if output.chars().count() > 200 { "..." } else { "" };
            summary.push_str(&format!("**Step {} [{}]**: {}\n> {}{}\n\n",
                step_id, status, step.desc, preview, suffix));
        }
        if chain.has_failure() { summary.push_str("\nSome steps failed.\n"); }

        self.messages.push(Message::user(&format!("Plan execution complete. Results:\n{}", summary)));
        let response = self.llm.chat(&self.messages, None, Some(0.7)).await?;
        let final_text = response.choices.into_iter().next()
    ...
    pub fn last_plan(&self) -> Option<&str> { self.last_plan.as_deref() }

    pub fn memory(&self) -> &MemorySearch { &self.memory }
    ...
    pub async fn shutdown(&mut self) {
        ...
        if self.iteration_count <= 2 { return; }
        ...
        if first_user.trim().is_empty() || last_assistant.trim().is_empty() { return; }
...
fn truncate(s: &str, n: usize) -> String {
    ...
    if s.chars().count() > n { format!("{}…", taken) } else { taken }
}
...
fn extract_plan(text: &str) -> Option<ToolCallChain> {
    ...
    if parsed.get("type")?.as_str()? != "plan" { return None; }
    ...
    for (i, step) in steps.iter().enumerate() {
        chain.add_step(
            ...
            step.get("depends_on").and_then(|d| d.as_array())
